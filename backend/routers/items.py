...
def list_items(db: Session = Depends(get_db), current_user: models.User = Depends(get_approved_user)):
    ...
    return db.query(models.Item).filter(models.Item.retired == False).all()
...
def create_item(item: schemas.ItemCreate, db: Session = Depends(get_db), admin: models.User = Depends(get_admin_user)):
    ...
    db.add(db_item)
    db.add(models.AuditLog(user_id=admin.id, action="item_created", details=f"Item: {item.name}"))
    db.commit()
    db.refresh(db_item)
    return db_item
...
def update_item(item_id: int, item: schemas.ItemUpdate, db: Session = Depends(get_db), admin: models.User = Depends(get_admin_user)):
    ...
    for k, v in item.dict(exclude_none=True).items():
        setattr(db_item, k, v)
    db.add(models.AuditLog(user_id=admin.id, action="item_updated", details=f"Item {item_id}"))
    db.commit()
    db.refresh(db_item)
    return db_item
...
def delete_item(item_id: int, db: Session = Depends(get_db), admin: models.User = Depends(get_admin_user)):
    ...
    if not db_item:
        raise HTTPException(status_code=404, detail="Item not found")
    db.delete(db_item)
    db.add(models.AuditLog(user_id=admin.id, action="item_deleted", details=f"Item {item_id}"))
    db.commit()
